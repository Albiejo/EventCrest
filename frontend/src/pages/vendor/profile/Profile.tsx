...
const Profile = () => {
  ...
  useEffect(() => {
      if(id){
        axiosInstanceVendor.get(`/getVendor?Id=${id}`,{withCredentials:true}).then((response) => {
        ...
        axiosInstanceVendor.get(`/getVendor?Id=${vendorData?._id}`,{withCredentials:true}).then((response) => {
          setVendor(response.data.data);
        }).catch((error) => {
          console.log("here", error);
        });
      ...
      <div className="overflow-hidden rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        ...
        <div className="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5">
          <div className="relative z-30 mx-auto -mt-16 h-30 w-full max-w-30 rounded-full bg-white/20 p-1 backdrop-blur sm:h-44 sm:max-w-44 sm:p-3">
            ...
            <img src={vendor?.logoUrl} alt="profile" className="rounded-full w-full h-30 object-cover" />
